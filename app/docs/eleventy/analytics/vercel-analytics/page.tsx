        ...
        <div className="space-y-8">
          ...
          <div>
            ...
            <ol className="space-y-2 text-slate-700 ml-11 list-decimal list-inside">
              <li>Make sure your Eleventy site is deployed on <a href="https://vercel.com" target="_blank" rel="noopener noreferrer" className="text-slate-600 underline font-semibold">Vercel</a></li>
          ...
          <div>
            ...
            <p className="text-slate-600 mb-4 ml-11">
              Eleventy outputs plain HTML, so you use Vercel&apos;s framework-agnostic snippet instead of a package. Add it right before <code className="bg-slate-100 px-2 py-0.5 rounded">&lt;/body&gt;</code> in <code className="bg-slate-100 px-2 py-0.5 rounded">src/_includes/layouts/base.html</code>. The <code className="bg-slate-100 px-2 py-0.5 rounded">{`{% if client.isProduction %}`}</code> guard skips it in local dev, where <code className="bg-slate-100 px-2 py-0.5 rounded">/_vercel/insights/script.js</code> isn&apos;t served:
            </p>
        ...
        <div className="space-y-6">
          <div>
            <p className="text-slate-600 mb-4">
              The script registers a global <code className="bg-slate-100 px-2 py-0.5 rounded">window.va</code> queue. Call it from any event handler — guard with <code className="bg-slate-100 px-2 py-0.5 rounded">window.va</code> so nothing throws in local dev where the script isn&apos;t loaded:
            </p>
  ...
  document.getElementById("signup")?.addEventListener("click", () => {
    window.va && window.va("event", { name: "Signup", data: { plan: "pro" } });
            ...
            <p className="text-sm text-slate-500 mt-3">
              Custom events require a Vercel Pro or Enterprise plan. The first argument is always <code className="bg-slate-100 px-1.5 py-0.5 rounded">&quot;event&quot;</code>; the object holds the event <code className="bg-slate-100 px-1.5 py-0.5 rounded">name</code> and optional <code className="bg-slate-100 px-1.5 py-0.5 rounded">data</code>.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-12 bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
        ...
        <div className="space-y-6">
          <p className="text-slate-600">
            Speed Insights is a separate, equally simple add-on. Enable it under the <span className="font-semibold">Speed Insights</span> tab in your Vercel project, then add its script alongside the analytics snippet in <code className="bg-slate-100 px-2 py-0.5 rounded">base.html</code>:
          </p>
