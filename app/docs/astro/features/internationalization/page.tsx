        ...
        <div className="space-y-8">
          ...
          <div>
            ...
            <p className="text-slate-600 mb-4 ml-11">
              Open <code className="bg-slate-100 px-2 py-1 rounded">astro.config.mjs</code> and add an
              {" "}<code className="bg-slate-100 px-2 py-0.5 rounded">i18n</code> object. List every
              supported language in <code className="bg-slate-100 px-2 py-0.5 rounded">locales</code> and
              pick one as your <code className="bg-slate-100 px-2 py-0.5 rounded">defaultLocale</code>:
            </p>
  ...
  i18n: {
    ...
    defaultLocale: "en",
          ...
          <div>
            ...
            <p className="text-slate-600 mb-4 ml-11">
              Organize content into <code className="bg-slate-100 px-2 py-0.5 rounded">/[locale]/</code> folders
              inside <code className="bg-slate-100 px-2 py-0.5 rounded">src/pages/</code>. Folder names must match
              your <code className="bg-slate-100 px-2 py-0.5 rounded">locales</code> exactly. With the default
              {" "}<code className="bg-slate-100 px-2 py-0.5 rounded">prefixDefaultLocale: false</code>, your
    ...
    └── pt-br/
        ...
        └── index.astro      → example.com/pt-br/`, 2)}
          ...
          <div>
            ...
            <p className="text-slate-600 mb-4 ml-11">
              Use the helper functions from the <code className="bg-slate-100 px-2 py-0.5 rounded">astro:i18n</code> module
              to compute correct, localized routes. <code className="bg-slate-100 px-2 py-0.5 rounded">getRelativeLocaleUrl()</code> always
      ...
      <div className="mb-12 bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-slate-50 rounded-xl p-6 border border-slate-200">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-slate-200 text-slate-700 rounded-full text-xs font-bold mb-3">
              Default
            </div>
            ...
            <p className="text-sm text-slate-600 mb-3">
              ...
              {" "}<code className="bg-slate-100 px-1.5 py-0.5 rounded">src/pages/</code>.
            </p>
            <ul className="space-y-2 text-sm text-slate-700">
              <li className="flex items-start gap-2">
                <span className="font-bold text-slate-600">•</span>
                <span><code className="bg-slate-100 px-1.5 py-0.5 rounded">pages/about.astro</code> → <code className="bg-slate-100 px-1.5 py-0.5 rounded">/about/</code></span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-bold text-slate-600">•</span>
                <span><code className="bg-slate-100 px-1.5 py-0.5 rounded">pages/fr/about.astro</code> → <code className="bg-slate-100 px-1.5 py-0.5 rounded">/fr/about/</code></span>
              </li>
            </ul>
          </div>

          <div className="bg-blue-50 rounded-xl p-6 border border-blue-200">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-200 text-slate-800 rounded-full text-xs font-bold mb-3">
              Prefixed
            </div>
            <h3 className="text-lg font-bold text-slate-900 mb-2">
        ...
        <div className="mt-6 bg-slate-900 rounded-lg p-4 font-mono text-xs text-slate-100 relative group">
          <pre className="whitespace-pre-wrap"><code>{`// astro.config.mjs
import { defineConfig } from "astro/config";
    ...
    routing: {
      ...
      <div className="mb-12 bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
        ...
        <p className="text-slate-600 mb-4">
          Map languages to a fallback locale with <code className="bg-slate-100 px-2 py-0.5 rounded">i18n.fallback</code>,
          then choose how the fallback is served with <code className="bg-slate-100 px-2 py-0.5 rounded">routing.fallbackType</code>:
          {" "}<code className="bg-slate-100 px-2 py-0.5 rounded">&quot;redirect&quot;</code> (default) sends the visitor to the
          fallback route, while <code className="bg-slate-100 px-2 py-0.5 rounded">&quot;rewrite&quot;</code> serves the fallback
  ...
  i18n: {
    ...
    routing: {
      ...
      <div className="mb-12 bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
        ...
        <p className="text-slate-600 mb-4">
          Instead of a plain string, pass an object with a <code className="bg-slate-100 px-2 py-0.5 rounded">path</code> (the
          URL prefix and folder name) and <code className="bg-slate-100 px-2 py-0.5 rounded">codes</code> (the browser
          language codes it covers). Useful for grouping variants like <code className="bg-slate-100 px-2 py-0.5 rounded">fr</code>,
          {" "}<code className="bg-slate-100 px-2 py-0.5 rounded">fr-CA</code>, and <code className="bg-slate-100 px-2 py-0.5 rounded">fr-BR</code> under
          one <code className="bg-slate-100 px-2 py-0.5 rounded">/french/</code> URL:
        </p>
      ...
      <div className="mb-12 bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-3 flex-wrap">
            ...
            <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 text-xs font-semibold border border-slate-200">
              astro@4.6.0+
            </span>
          </div>
          <p className="text-slate-600 mb-4">
            Set <code className="bg-slate-100 px-2 py-0.5 rounded">routing: &quot;manual&quot;</code> to disable Astro&apos;s i18n
            middleware and write your own. No other routing options can be combined with it. Astro exposes
            {" "}<code className="bg-slate-100 px-2 py-0.5 rounded">redirectToDefaultLocale()</code>, <code className="bg-slate-100 px-2 py-0.5 rounded">notFound()</code>,
            and <code className="bg-slate-100 px-2 py-0.5 rounded">redirectToFallback()</code> for your middleware:
          </p>
  ...
  return redirectToDefaultLocale(302);
        ...
        <div>
          <h3 className="text-xl font-bold text-slate-900 mb-3">
            The <code className="bg-slate-100 px-2 py-1 rounded text-lg">middleware()</code> Function
          </h3>
          <p className="text-slate-600 mb-4">
            To <em>extend</em> rather than replace Astro&apos;s i18n routing, create its middleware manually with
            {" "}<code className="bg-slate-100 px-2 py-0.5 rounded">middleware()</code> and order it against your own using
            {" "}<code className="bg-slate-100 px-2 py-0.5 rounded">sequence()</code>:
          </p>
  ...
  middleware({
    ...
    fallbackType: "redirect",
      ...
      <div className="mb-12 bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
        ...
        <p className="text-slate-600 mb-4">
          For server output (with the <code className="bg-slate-100 px-2 py-0.5 rounded">@astrojs/node</code> or
          {" "}<code className="bg-slate-100 px-2 py-0.5 rounded">@astrojs/vercel</code> adapter and a configured
          {" "}<code className="bg-slate-100 px-2 py-0.5 rounded">site</code>), map locales to custom domains with
          {" "}<code className="bg-slate-100 px-2 py-0.5 rounded">i18n.domains</code>. Unmapped locales keep your
          {" "}<code className="bg-slate-100 px-2 py-0.5 rounded">prefixDefaultLocale</code> behavior:
        </p>
...
// /about.astro    → https://example.com/about`, 9)}
        ...
        <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-sm text-slate-900 font-medium mb-2">⚙️ Requirements &amp; limitations</p>
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-xl p-5 border border-blue-200">
            ...
            <p className="text-sm text-slate-600">
              The best-matching locale for the visitor, or <code className="bg-slate-100 px-1 rounded">undefined</code> if none match.
            </p>
          </div>
          <div className="bg-white rounded-xl p-5 border border-blue-200">
            <h3 className="font-bold text-slate-900 mb-1 text-sm">
