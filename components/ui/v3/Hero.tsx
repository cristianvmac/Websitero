...
import { Sparkles, ArrowRight, Star, Zap, ShieldCheck, Check } from "lucide-react";
import { BRAND_GRADIENT, BRAND_TEXT, GridBackdrop } from "./_ui";
...
const Hero = () => {
  ...
  return (
    <section id="hero" className="relative overflow-hidden bg-linear-to-b from-slate-50 to-white px-6 py-16 lg:py-24">
      <GridBackdrop />
      <div className="pointer-events-none absolute -top-24 left-1/4 h-72 w-72 rounded-full bg-[#4588ba]/15 blur-3xl" />
      <div className="pointer-events-none absolute -bottom-24 right-1/4 h-72 w-72 rounded-full bg-[#8b5cf6]/10 blur-3xl" />
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col items-center gap-16 lg:flex-row lg:gap-20">
        ...
        <div className="flex flex-col items-center gap-7 text-center lg:items-start lg:text-left">
          ...
          <div className="flex flex-col items-center gap-3 sm:flex-row">
            <Link
              href="#cta"
              className={`inline-flex items-center gap-2 rounded-full ${BRAND_GRADIENT} px-7 py-3.5 text-base font-semibold text-white shadow-lg shadow-[#4588ba]/30 transition-all hover:-translate-y-0.5 hover:shadow-xl hover:shadow-[#4588ba]/40`}
            >
              Get Websitero
        ...
        {/* Right — browser + terminal mock */}
        <div className="relative w-full max-w-lg lg:w-1/2">
          <div className="absolute -inset-4 rounded-[2rem] bg-linear-to-br from-[#4588ba]/20 to-[#8b5cf6]/10 blur-2xl" />
          ...
          {/* Terminal card (reacts to framework toggle) */}
          <div className="absolute -bottom-6 -left-4 right-8 rounded-xl bg-slate-900 p-4 font-mono text-xs text-slate-100 shadow-xl">
            <div className="flex items-center gap-2 text-slate-400">
              <span style={{ color: active.accent }}>❯</span>
              <span className="truncate">{active.command}</span>
              <span className="ml-auto inline-block h-3.5 w-1.5 animate-pulse bg-slate-300" />
            </div>
          </div>
          ...
          <div className="absolute -right-4 -top-4 hidden items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-xl sm:flex">
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-slate-500">Lighthouse</p>
            </div>
          </div>
        </div>
      </div>
