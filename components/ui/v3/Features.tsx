...
const cells: Cell[] = [
  ...
  },
  {
    icon: Search,
    title: "SEO services",
    desc: "Keyword strategy, on-page optimization, and performance tuning so customers actually find you.",
  },
  {
    icon: MapPin,
    title: "Google Business Profile",
    desc: "Profile setup, review management, and local SEO to own the map pack.",
    tool: "Google",
  },
  {
    icon: Server,
    title: "Hosting",
    desc: "Fast, reliable hosting with SSL and 99.9% uptime — deploy in minutes.",
    tool: "Netlify",
  },
  {
    icon: Globe,
    title: "Domains",
    desc: "Help picking the perfect name, plus registration and DNS setup.",
    tool: "Namecheap",
  },
  {
    icon: Smartphone,
    title: "Mobile first",
    desc: "Touch-friendly, fast-loading layouts that shine on every screen size.",
  },
  {
    icon: Headphones,
    title: "Ongoing support",
    desc: "Active Discord community, analytics, and continuous content improvements.",
    tool: "Discord",
  },
];
          ...
          {cells.map((cell) => {
            ...
            if (cell.featured) {
              return (
                <article
                  key={cell.title}
                  className={`group relative flex flex-col justify-between overflow-hidden rounded-2xl ${BRAND_GRADIENT} p-8 text-white shadow-lg shadow-[#4588ba]/25 ${cell.span ?? ""}`}
                >
                  <div
                    aria-hidden
            ...
            return (
              <article
                key={cell.title}
                className={`group relative flex flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:border-[#4588ba]/40 hover:shadow-lg ${cell.span ?? ""}`}
              >
